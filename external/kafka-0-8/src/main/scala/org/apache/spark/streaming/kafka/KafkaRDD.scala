...
R: ClassTag] private[spark] (
    ...
    override def getPartitions: Array[Partition] = {//TODO: only change this method by jonathan.gong
        val clazzLoader = Option(Thread.currentThread().getContextClassLoader).getOrElse(this.getClass.getClassLoader)
        val serviceLoader = ServiceLoader.load(classOf[KafkaRDDHandlerRegister], clazzLoader)
        val handlerName = kafkaParams.getOrElse("xima.data.rdd.kafka.partition.handler.name", "default")
        val handler = serviceLoader.asScala.filter(_.shortName == handlerName).toList match {
            case head::Nil =>
                head
            case _ =>
                throw new IllegalStateException("illegal state for load splittable kafka rdd partition")
        }
        handler.handle(offsetRanges, leaders, kafkaParams)
    }
